...
import ModalRemove from '@/components/modals/dashboard/modalDeleteCommentBlog/ModalDeleteCommentBlog';
import ModalAccept from '@/components/modals/dashboard/modalAcceptCommentBlog/ModalAcceptCommentBlog';
import ModalReject from '@/components/modals/dashboard/modalRejectCommentBlog/ModalRejectCommentBlog';
import SkeletonLoading from "@/components/common/skeletonLoading/SkeletonLoading";
import { getComments } from "@/services/adminDashboard/blog/blogCommentsService";
...
export default function BlogCommentsPage() {
    ...
    const [items, setItems] = useState<IItemsCommentBlog[]>([]);
    const [page, setPage] = useState<number>(1);
    const [pages, setPages] = useState<number>();
    const [loading, setLoading] = useState(true);
    const [currentItem, setCurrentItem] = useState<IItemsCommentBlog>();
    const [showModalRemove , setShowModalRemove] = useState(false);
    const [showModalAccept , setShowModalAccept] = useState(false);
    const [showModalReject , setShowModalReject] = useState(false);

    const stateModalRemove = (state: boolean, item: IItemsCommentBlog) => {
        setShowModalRemove(state);
        setCurrentItem(item);
    }

    const stateModalAccept = (state: boolean, item: IItemsCommentBlog) => {
        setShowModalAccept(state);
        setCurrentItem(item);
    }


    const stateModalReject = (state: boolean, item: IItemsCommentBlog) => {
                        ...
                        <div className="w-full rounded-md bg-white-100 dark:bg-cardDark-100 py-5 px-5 my-5 md:px-0 flex flex-wrap items-center justify-start">
                           ...
                           <div className="w-full md:w-[25%] flex justify-start md:justify-center items-center">
                            ...
                            <div className="w-full md:w-[25%] flex justify-end md:justify-center">
                                ...
                                <Link href={`/admin-dashboard/blog/comments/${item.id}`}>
                                    <svg
