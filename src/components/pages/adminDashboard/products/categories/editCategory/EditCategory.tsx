...
import { updateCategory } from "@/services/adminDashboard/products/categoriesService";
...
export default function EdiitCategoryPage({
    ...
    formItems: [
      ...
      },
      {
        ...
        config: {
          label: "دسته بندی پدر",
          name: "parent",
          options: [{ id: null, title: "بدون دسته بندی پدر" }, ...categories.map(cat=>({id: cat.id , title: cat.title}))],
          classes: "w-full",
        },
        value: category.parent?.id ?? null,
        validation: {
          selectRequired: true,
        },
        valid: false,
      ...
      },
      {
        ...
        config: {
          ...
          name: "attributes",
          classes: "w-full",
          options: allAttributes && allAttributes.map((item: IAttribute) => ({
            id : item.id ,
            title: item.title
          }))
        },
        value: category.attributes?.map((item: IAttribute) => (item.id )) || [],
        validation: {
        },
        valid: false,
        errorMsg: "",
  ...
  const submitHandler = async (form: IProductCat) => {
    setLoadingBtn(true);
    try {
    ...
    <div className="container mx-auto p-3">
      ...
      {/* <Card title="ویژگی‌ها" classes="w-[90%] max-w-[1000px] mx-auto">
        ...
        attributes && attributes.map((attribute, index) =>(
            <div key={index} className="w-full rounded-md bg-white-100 dark:bg-cardDark-100 py-5 px-5 my-5 md:px-0 flex flex-wrap items-center justify-start">
