...
import ModalRemove from '@/components/modals/dashboard/modalDeleteProductCat/ModalDeleteProductCat';
...
export default function CategoriesPage() {
  ...
  const doneRemove = (id: number) => {
    const removeFromArray = (array: ICategory[]): ICategory[] => {
      return array.filter(item => {
        // اگر آیتم فعلی همان آیتمی است که باید حذف شود
        if (item.id === id) {
          return false;
        }
        
        // اگر آیتم فرزندانی دارد، آنها را نیز بررسی کن
    ...
    const newItems = removeFromArray([...items]);
    setItems(newItems);
  }
  ...
  return (
    ...
    <div className="container mx-auto p-3">
      <div className="flex items-center justify-between mb-10">
        ...
        <Link
          href={"/admin-dashboard/product-cats/new"}
      ...
      {loading ? (
        <SkeletonLoading rows={5} cols={1} itemClasses={"h-[70px]"} />
      ) : (
        <>
          {items &&
            items.map((cat, index) => {
              return (
                <div key={index}>
                  <div className="w-full">
                    ...
                    <div className="w-full rounded-md bg-white-100 dark:bg-cardDark-100 py-5 px-5 my-5 md:px-0 flex flex-wrap items-center justify-start">
                      ...
                      <div className="w-full md:w-[10%] flex justify-end md:justify-center">
                        <Link href={`/admin-dashboard/product-cats/${cat.id}`}>
                          <svg
                  ...
                  {/* نمایش فرزندان اگر وجود داشتند */}
                  {cat.children?.map(
                    (child: ICategory, childIndex: number) => (
                      <div key={childIndex} className="w-full md:pr-[40px]">
                        {/* دسته‌بندی پدر */}
                        <div className="w-full rounded-md bg-white-100 dark:bg-cardDark-100 py-5 px-5 my-5 md:px-0 flex flex-wrap items-center justify-start">
                          <div className="w-full md:w-[30%] flex justify-start md:justify-center items-center">
                            <span className="text-textPrimary-100 dark:text-white-50 font-semibold md:hidden">
                          ...
                          <div className="w-full md:w-[10%] flex justify-end md:justify-center">
                            <Link href={`/admin-dashboard/product-cats/${child.id}`}>
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                fill="none"
                                viewBox="0 0 24 24"
                            ...
                            <svg
                              ...
                              stroke="currentColor"
                              className="w-4 h-4 md:w-6 md:h-6 mr-1 md:hover:stroke-primary-100 cursor-pointer"
                              onClick={()=>stateModalRemove(true , child)}
                            >
                              <path
                                strokeLinecap="round"
                                strokeLinejoin="round"
