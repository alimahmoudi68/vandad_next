...
export default function AttributesMetaPage({
  permissions,
  attributesMeta: initialAttributesMeta,
}: AttributesPageProps) {

  const router = useRouter();
  const dispatch = useDispatch();
  const { attributesMeta, loading } = useSelector(
    (state: RootState) => state.attributesMetaAdmin
  );

  useEffect(() => {
    dispatch(setLoading()); 
    dispatch(updateAttributesMetaAdmin(initialAttributesMeta));
  }, [initialAttributesMeta, dispatch]);
  ...
  return (
    <div className="container mx-auto p-3">
      <div className="flex items-center justify-between mb-10">
        ...
        <Link
          href={"/dashboard/attributesMeta/new"}
      ...
      {loading ? (
        ...
        <>
              ...
              return (
                <div key={index} className="w-full">
                  <div className="w-full rounded-md bg-white-100 dark:bg-cardDark-100 py-5 px-5 my-5 md:px-0 flex flex-wrap items-center justify-between">
                    ...
                    <Link href={`/dashboard/attributesMeta/${attr._id}`}>
                      <svg
                      ...
                      <Link href={`/dashboard/attributesMeta/delete/${attr._id}`}>
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          fill="none"
                          viewBox="0 0 24 24"
          ...
          {attributesMeta && attributesMeta.length === 0 && (
