...
import { removeAttributeApi } from "@/services/dashboard/products/attributesService";
...
export default function DeleteAttributeModal({ id }: { id: string }) {
  ...
  const handleConfirm = async () => {
    if (!loading) {
      try {
        setLoading(true);
        const data = await removeAttributeApi(id);
        setLoading(false);
  ...
  return (
    <MasterModal title="حذف ویژگی محصول" close={handleCancel}>
      <span className="dark:text-white-100 text-textPrimary-100">
        آیا از حذف ویژگی محصول مطمئن هستید؟
      </span>
