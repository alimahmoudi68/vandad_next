...
import {acceptComment} from '@/services/adminDashboard/tv/tvCommentsService';
import { IItemsCommentTv } from "@/types/tvComment";


interface ModalAcceptCommentTvProps {
  item: IItemsCommentTv;
  close: () => void;
  done: (id: number) => void;
}

const ModalAcceptCommentTv: React.FC<ModalAcceptCommentTvProps> = ({ item, close, done }) => {
  ...
  const removeHandler = async () => {
      if (!loading) {
        try {
          setLoading(true);
          const data  = await acceptComment(item.id);
  ...
  return (
    <>
      <MasterModal close={close} title={'تایید نظر'}>
...
export default ModalAcceptCommentTv;
